import { IDENTIFIER, arrayAt, assertAcyclicDependencies, fail, normalizedContract, objectAt, parseIdentifierList, parsePlanCommand, stringAt, } from "./validation.js";
export const EXECUTION_PLAN_SCHEMA_V2 = "openthrottle.execution-plan/v2";
export const EXECUTION_PLAN_V2_MAX_BYTES = 256 * 1024;
function parseTextList(value, path, options) {
    return arrayAt(value, path, (entry, entryPath) => {
        return stringAt(entry, entryPath, { max: options.itemMax });
    }, options);
}
function parseUnitV2(value, path) {
...
function validatePlanV2(plan, source) {
    const units = new Map(plan.units.map((unit) => [unit.id, unit]));
    if (units.size !== plan.units.length)
        fail(`${source}.units`, "must not contain duplicate IDs");
    for (const unit of plan.units) {
        for (const dependency of unit.depends_on) {
            if (!units.has(dependency))
                fail(`${source}.units.${unit.id}.depends_on`, "references an unknown unit");
        }
    }
...
export function validateExecutionPlanContractV2(value, options = {}) {
    const source = options.source ?? "execution_plan";
    const input = objectAt(value, source, ["schema", "pipeline_id", "plan_id", "units", "commands"]);
    if (input.schema !== EXECUTION_PLAN_SCHEMA_V2)
        fail(`${source}.schema`, `must be ${EXECUTION_PLAN_SCHEMA_V2}`);
    const plan = {
...
export function parseExecutionPlanContractV2(raw, options = {}) {
    if (Buffer.byteLength(raw, "utf8") > EXECUTION_PLAN_V2_MAX_BYTES) {
        fail(options.source ?? "execution_plan", "JSON exceeds 256 KiB");
    }
    return validateExecutionPlanContractV2(JSON.parse(raw), options);
}
