...
import type { ExecutionRecordPayloadContract } from "./records.js";
...
export const EVIDENCE_ARTIFACT_DESCRIPTOR_SCHEMA =
  "openthrottle.evidence-artifact-descriptor/v1" as const;
...
export type AttemptEvidencePayloadSchema =
  (typeof ATTEMPT_EVIDENCE_PAYLOAD_SCHEMAS)[number];

export interface AttemptEvidenceDiagnostic {
  path: string;
  detail: string;
}
...
export interface InvalidResultEvidencePayload {
  ...
  diagnostics: AttemptEvidenceDiagnostic[];
  runner_stdout_tail: string;
  runner_stderr_tail: string;
  observed_at: string;
}
...
export interface EvidenceArtifactDescriptor {
  schema: typeof EVIDENCE_ARTIFACT_DESCRIPTOR_SCHEMA;
  file: string;
  sha256: string;
  bytes: number;
  media_type: "application/json";
  payload_schema: AttemptEvidencePayloadSchema;
}
...
function boundedText(value: unknown, path: string, maximum: number): string {
  ...
  if (value.includes("\0")) fail(path, "must not contain NUL characters");
  return value;
}

function diagnostic(value: unknown, path: string): AttemptEvidenceDiagnostic {
...
export function validateAttemptForensicsPayload(
  value: unknown,
  options: { source?: string } = {},
): ValidatedContract<AttemptForensicsPayload> {
  ...
  return normalizedContract({
    ...
    definition_bundle_hash: digest(
      input.definition_bundle_hash,
      `${source}.definition_bundle_hash`,
    ),
    lease_id: id(input.lease_id, `${source}.lease_id`),
    work_retry_ordinal: integerAt(
      input.work_retry_ordinal,
      `${source}.work_retry_ordinal`,
      0,
      Number.MAX_SAFE_INTEGER,
    ),
    operational_signature: digest(
      input.operational_signature,
      `${source}.operational_signature`,
    ),
    exit_code: integerAt(input.exit_code, `${source}.exit_code`, 0, 255),
    runner_stdout_tail: boundedText(
      input.runner_stdout_tail,
      `${source}.runner_stdout_tail`,
      EVIDENCE_TAIL_MAX_CHARACTERS,
    ),
    runner_stderr_tail: boundedText(
      input.runner_stderr_tail,
      `${source}.runner_stderr_tail`,
      EVIDENCE_TAIL_MAX_CHARACTERS,
    ),
    ...
    workspace_git_status: jsonValueAt(
      input.workspace_git_status,
      `${source}.workspace_git_status`,
    ),
    observed_at: timestampAt(input.observed_at, `${source}.observed_at`, { normalize: false }),
  });
}

export function validateInvalidResultEvidencePayload(
  value: unknown,
  options: { source?: string } = {},
): ValidatedContract<InvalidResultEvidencePayload> {
  ...
  return normalizedContract({
    ...
    phase: enumAt(input.phase, `${source}.phase`, ["work", "result_correction"] as const),
    candidate_hash: nullable(input.candidate_hash, (entry) =>
      digest(entry, `${source}.candidate_hash`)),
    rejected_candidate: nullable(input.rejected_candidate, (entry) =>
      jsonValueAt(entry, `${source}.rejected_candidate`)),
    diagnostics: arrayAt(
      input.diagnostics,
      `${source}.diagnostics`,
      diagnostic,
...
export function validateAttemptEvidencePayload(
  value: unknown,
  options: { source?: string } = {},
): ValidatedContract<AttemptEvidencePayload> {
...
export function validateEvidenceArtifactDescriptor(
  value: unknown,
  options: { source?: string; payloadSchema?: AttemptEvidencePayloadSchema } = {},
): ValidatedContract<EvidenceArtifactDescriptor> {
...
export const ATTEMPT_FORENSICS_PAYLOAD_CONTRACT: ExecutionRecordPayloadContract = Object.freeze({
  ...
  parseInline: (value: unknown, path: string): AttemptForensicsPayload =>
    validateAttemptForensicsPayload(value, { source: path }).value,
});

export const INVALID_RESULT_EVIDENCE_PAYLOAD_CONTRACT: ExecutionRecordPayloadContract = Object.freeze({
  kind: "decision" as const,
  parseInline: (value: unknown, path: string): InvalidResultEvidencePayload =>
    validateInvalidResultEvidencePayload(value, { source: path }).value,
});
