...
function parseStringList(value: unknown, path: string, max: number): string[] {
...
function parseMcpServer(value: unknown, path: string): ConfigMcpServer {
  const input = objectAt(value, path, ["command", "args", "env"]);
  return {
    command: stringAt(input.command, `${path}.command`, { max: 1_000 }),
    args: input.args === undefined ? [] : parseStringList(input.args, `${path}.args`, 64),
    env: input.env === undefined
      ? {}
      : recordAt(input.env, `${path}.env`, (entry, entryPath) => stringAt(entry, entryPath, { max: 1_000 }), {
        max: 64,
...
export function validateRepositoryConfigContract(
  ...
  const config: RepositoryConfigContract = {
    ...
    ...(input.pipelines === undefined ? {} : { pipelines: parseStringMap(input.pipelines, `${source}.pipelines`) }),
