...
- Changes to `supervisor/src/persistence/migrations/definitions.ts` pause and
  drain admission before deploy and require the live pre-deploy supervisor's
  cutover evidence to advertise
...
for changes to `supervisor/src/persistence/migrations/definitions.ts` before
the new supervisor image can open SQLite. Unmarked or malformed future ledger
rows remain startup-fatal for rollback safety.
