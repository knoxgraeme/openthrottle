import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { openDb } from "./database.js";
import { createSupervisorStore, type SupervisorStore } from "./store.js";

describe("settings store", () => {
  let db: ReturnType<typeof openDb>;
  let store: SupervisorStore;

  beforeEach(() => {
    db = openDb(":memory:");
    store = createSupervisorStore(db);
  });

  afterEach(() => db.close());

  it("persists operator settings", () => {
    store.setSetting("catalog-digest", "abc123");
    expect(store.getSetting("catalog-digest")).toBe("abc123");
  });

  it("prunes only rows under the requested prefix that were written before the cutoff", () => {
    store.setSetting("github-supervisor-comment:100", "control-session");
    store.setSetting("github-supervisor-comment:200", "pipeline-status");
    store.setSetting("github-head:issue-1", "a".repeat(40));
    db.prepare("UPDATE settings SET updated_at = ? WHERE key = ?")
      .run("2020-01-01T00:00:00.000Z", "github-supervisor-comment:100");
    db.prepare("UPDATE settings SET updated_at = ? WHERE key = ?")
      .run("2020-01-01T00:00:00.000Z", "github-head:issue-1");

    expect(store.pruneSettings("github-supervisor-comment:", "2021-01-01T00:00:00.000Z")).toBe(1);

    expect(store.getSetting("github-supervisor-comment:100")).toBeUndefined();
    expect(store.getSetting("github-supervisor-comment:200")).toBe("pipeline-status");
    // Rows outside the prefix are untouched regardless of age.
    expect(store.getSetting("github-head:issue-1")).toBe("a".repeat(40));
  });

  it("retains pre-migration rows with no write timestamp until they are rewritten", () => {
    db.prepare("INSERT INTO settings (key, value) VALUES (?, ?)")
      .run("github-supervisor-comment:1", "control-session");

    expect(store.pruneSettings("github-supervisor-comment:", "2999-01-01T00:00:00.000Z")).toBe(0);
    expect(store.getSetting("github-supervisor-comment:1")).toBe("control-session");

    // A rewrite stamps the row, after which retention applies normally.
    store.setSetting("github-supervisor-comment:1", "control-session");
    expect(store.pruneSettings("github-supervisor-comment:", "2999-01-01T00:00:00.000Z")).toBe(1);
    expect(store.getSetting("github-supervisor-comment:1")).toBeUndefined();
  });

  it("stamps the write time through both single and batched setters", () => {
    store.setSetting("stamped:single", "value");
    store.setSettings([{ key: "stamped:batch", value: "value" }]);
    const rows = db.prepare(
      "SELECT key, updated_at FROM settings WHERE key >= 'stamped:' ORDER BY key"
    ).all() as Array<{ key: string; updated_at: string | null }>;
    expect(rows).toHaveLength(2);
    for (const row of rows) {
      expect(row.updated_at).toBeTruthy();
      expect(Number.isNaN(Date.parse(row.updated_at!))).toBe(false);
    }
  });

  it("commits a coherent setting projection atomically", () => {
    db.exec(`
      CREATE TRIGGER reject_incomplete_projection
      BEFORE INSERT ON settings
      WHEN NEW.key = 'projection:reject'
      BEGIN
        SELECT RAISE(ABORT, 'projection rejected');
      END
    `);

    expect(() => store.setSettings([
      { key: "projection:head", value: "head-b" },
      { key: "projection:reject", value: "authoritative" },
      { key: "projection:observed-at", value: "2026-01-01T00:00:00.000Z" },
    ])).toThrow("projection rejected");
    expect(store.listSettings("projection:")).toEqual([]);

    store.setSettings([
      { key: "projection:head", value: "head-b" },
      { key: "projection:source", value: "authoritative" },
      { key: "projection:observed-at", value: "2026-01-01T00:00:00.000Z" },
    ]);
    expect(store.listSettings("projection:")).toEqual([
      { key: "projection:head", value: "head-b" },
      { key: "projection:observed-at", value: "2026-01-01T00:00:00.000Z" },
      { key: "projection:source", value: "authoritative" },
    ]);
  });
});
