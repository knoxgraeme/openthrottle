...
/**
 * Snapshot the complete repository-authored definition tree at one exact Git
 * commit. This reader proves provenance and transport bounds only; it does not
 * interpret definitions or grant authority to reserved namespaces.
 */
export async function getRepositoryDefinitionSourceAtCommit(
  ...
  for (const entry of definitionTree) {
    ...
    for (let separator = caseKey.lastIndexOf("/"); separator > DEFINITION_ROOT.length; separator = caseKey.lastIndexOf("/", separator - 1)) {
