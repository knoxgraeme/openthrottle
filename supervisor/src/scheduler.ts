...
const TRIAGE_INSTRUCTIONS =
  ...
  "every open review thread and comment so you answer the complete review, not " +
  "one comment at a time. Then reply visibly on EVERY item on its own thread — " +
  "when you make a change, reply with what you did and the commit that addresses " +
  "it and resolve the thread; when no change is needed, reply with your " +
  "reasoning. Batch decision-required items into one elicitation. Run the " +
  "repository's own test/lint/build gates on your fix, then push and end the run " +
  "— do NOT sit and watch remote CI in a loop. OpenThrottle watches CI for " +
  "you: any check that fails on your push is delivered back to this same session " +
  "as another follow-up, so a red check becomes a fresh resume, not a reason to " +
  "block here. Refresh the `## OpenThrottle gates` checklist in the PR " +
  "description to reflect the true state of every gate (tests, lint, build, CI, " +
  "review threads), marking anything you could not run — e.g. a gate the sandbox " +
  "OOM-killed — as a known gap, never as done, and marking CI as awaiting the " +
  "next automated check. Leave nothing unaddressed, and end with your " +
