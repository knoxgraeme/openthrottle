...
Ready units may work concurrently when their dependency IDs are settled. Each
Attempt has a stable scope/member identity and receives only the ordered Result,
Decision, and Checkpoint evidence of its dependencies. Unit workers cannot see
or modify sibling worktrees.
...
all units integrated
  ...
  -> persona_review(inspect fanout)
  -> validate_review_findings(inspect)
       |-- confirmed blockers --> final_repair(edit) --> final_test
       `-- clear -------------> publish(effect) -> provider(wait)
```

The selector chooses at most the sealed roster limit from the skill allowlist.
It cannot name a new agent or skill. Each chosen lens runs in a fresh inspect
Attempt against the same exact integrated subject. Current lenses include
...
- Result-shape correction preserves the unit Checkpoint and native session; it
  never reruns implementation merely to repair JSON.
