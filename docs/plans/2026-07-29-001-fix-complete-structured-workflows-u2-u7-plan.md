...
- RR5. `openthrottle init` and `ship --graph` must emit and validate the canonical config/graph selection. An explicit structured selection must never fall back to `simple`. Until the exact production `graph/for-each-unit@1` capability and dispatcher are reachable, structured ship must return a stable unavailable error before any Linear mutation; RU9 may activate mutation only after an authenticated supervisor capability check succeeds.
...
- RR15. The durable unit reducer must advance through implement, simplify, configured commands, executor candidate creation, lead acceptance bound to that candidate subject and its command receipts, integration, final commands, fresh final review, and bounded final repair without allowing an agent to commit, push, integrate, or pass a gate.
...
- RAE9. Given an explicit structured selection before RU9 is deployed, when ship validates the plan, then it returns a stable capability-unavailable error before any Linear mutation and does not silently delegate the simple graph.
- RAE10. Given commands passed for a unit subject, when acceptance is requested, then executor candidate evidence already binds that subject and the command receipts; the lead accepts that exact candidate before integration may run.
...
- KTD2. **Separate structured authoring from production activation.** RU2 makes preparation and deterministic validation usable, but an explicit structured ship remains hard-failed before Linear mutation. RU3 proves resolution and compilation only against test capability descriptors. RU9 atomically installs the runtime capability and dispatcher, exposes their authenticated capability evidence, and activates CLI mutation only when that evidence matches; missing, stale, or mismatched evidence stays fail closed. `simple` remains the repository default, and origin U8 owns the first credentialed invocation and any repository migration after RU11 passes.
...
When a unit changes a normative runtime contract, that unit must update `docs/SPEC.md` in the same PR; RU11 owns final convergence, not deferred correction of earlier contract changes.
...
| RU2 | Make plan preparation and structured ship selection safe | `cli/src/plan.ts`, `cli/src/ship.ts`, `cli/src/init.ts`, planning skill packaging | RU1 |
...
| RU9 | Compose and activate the child runtime with fail-closed aggregate | composition root, capability endpoint/CLI fence, operations/runtime adapters, aggregate settlement | RU3, RU5, RU8 |
...
### RU2. Make plan preparation and structured ship selection safe

- **Goal:** Finish the local authoring and validation entry point while keeping structured delegation unavailable until its production runtime exists.
...
- **Approach:** Replace the unconditional prepare error with configured local-engine invocation of the one canonical skill. Keep validate deterministic. Make init emit canonical simple defaults and discoverable structured selection. Parse and validate an explicit structured graph/execution block, never coerce it to `simple`, and then return a stable runtime-unavailable error before team lookup, issue creation, or delegation. RU9 exclusively owns removing this temporary hard failure in favor of authenticated capability discovery.
- **Test Scenarios:** Prepare updates exactly one execution-plan block. Missing engine/auth errors are actionable. Structured ship rejects missing/cyclic/invalid input with its validation error and rejects valid structured input with the stable unavailable error before any Linear GraphQL mutation. Simple ship remains compatible with a complete plan. Explicit structured selection never delegates `simple`.
...
- **Approach:** Replace legacy production pipeline selection with the shared config/graph contract. Ticket content may select only a repository-allowlisted graph name. Config allowlists committed repository skill directories by id, and graphs may reference them only as `repo://<skill-id>`; admission resolves each selected directory's `SKILL.md` package at the exact base commit and pins its canonical reference, invocation name, accepted regular-file blobs, and package digest. Reject symlinks, traversal, path escape, oversized or undeclared closure entries, and changed blobs. Compile repository-agent stages to `agent/repository-skill@1` against test descriptors while carrying repository skill identity in the compiled manifest/request. Compile `for_each_unit` to `graph/for-each-unit@1` against test capability descriptors. Install the immutable structured built-in while leaving repository defaults unchanged and production admission fail closed for the composite and repository-skill capabilities until RU9. Preserve the full `core/implement@4` simple parity oracle.
- **Test Scenarios:** Valid simple, structured, and repository-skill bundles compile and pin against test descriptors. Unknown selection, missing execution plan, changed blob, symlink, traversal, path escape, size excess, undeclared reference, unsupported capability, or digest mismatch fails before provisioning. Production descriptors reject the composite and repository-skill capabilities until RU9. Simple parity covers all stages, transitions, context policies, and three repair bounds.
...
- **Files:** additive migration definitions/schema/tests, `supervisor/src/persistence/pipeline/unit-store.ts`, `unit-store.test.ts`, `status-store.ts`, `supervisor/src/operations/unit-effects.ts`, runtime event/poller tests, `docs/SPEC.md`.
- **Approach:** Add composite foreign keys or equivalent composite constraints that prevent cross-instance parent-attempt bindings. Recovery first identifies the exact expired current action, then invokes idempotent runtime collection outside the SQLite transaction. A recovered result completes through a current-pointer compare-and-set. Only a confirmed no-result collection may heal through a separate current-pointer compare-and-set. Add an `execution_graph_id`-leading status index and query path. In the same PR, replace the normative immediate missed-heartbeat exit language in `docs/SPEC.md` with this reconcile-before-heal contract.
- **Test Scenarios:** Cross-instance and mixed-attempt inserts fail. A late durable result is collected once instead of exited. Concurrent collection/heal produces one terminal outcome. Collection errors retain the active action for bounded retry rather than proving no result. Status query uses the graph index and preserves authored order. The normative liveness text matches the tested transition order.
- **Verification:** Run migration, unit-store, status-store, runtime event/poller tests; supervisor typecheck/build; `git diff --check docs/SPEC.md`.
...
- **Approach:** Keep provider secret identifiers exclusively in the supervisor adapter. Map a closed logical-scope set to the minimal sandbox credential classes and reject operator-only Daytona, Fly, webhook, install, and supervisor credentials. Build engine, MCP, and repository-skill discovery configuration from a clean trusted baseline without importing personal local configuration. Materialize pinned repository skill packages only into the current action's private discovery directory, then remove action config, auth files, skill copies, caches, sockets, subprocess environment, and rotated state at completion, cancellation, failure, and reconciliation before the next action.
...
- **Approach:** Extend the standard receipt envelope with every graph/parent/action/session/producer/subject fence, including separate repository-skill package digest and installed runtime capability digest for repository-skill executions. Validate current exact-subject executor facts before gate evaluation. Invoke `ce-code-review` in report-only mode. Route any change through final repair and require new commands and fresh review.
...
- **Approach:** Widen the durable action-kind vocabulary and TypeScript union for every named unit and whole-change phase, preserving uniqueness and migration from current child tables. Then persist the exact unit order: implement, simplify, named commands, executor candidate derivation, lead decision bound to that candidate subject and its accepted command receipts, and only then integration. Persist unit repair, final commands, final review, and final repair as fenced transitions as well. Preserve one active action. Resume the correct unit or final-repair session within bounds and force fresh final review sessions.
- **Test Scenarios:** Unit success traverses every required phase. Command failure repairs then re-simplifies and reruns commands. No lead action is leaseable before current candidate evidence exists; lead `accepted_subject` must equal that candidate subject and reference the accepted command receipts. Integration is impossible before lead acceptance. Candidate/integration replay is idempotent. Final repair reruns full commands and fresh review. No agent result can skip a phase.
...
### RU9. Compose and activate the child runtime with fail-closed aggregate

- **Goal:** Make the phase machine production-reachable, activate structured shipping only against that reachable runtime, and settle the parent only from accepted integration evidence.
- **Requirements:** RR5, RR16-RR17.
- **Files:** `supervisor/src/index.ts`, `supervisor/src/http/server.ts`, `supervisor/pipelines/runtime-capabilities-v1.json`, sandbox capability descriptor, provider-neutral runtime/operations ports, Daytona adapter, coordinator/aggregate paths, `cli/src/ship.ts`, `cli/src/util.ts`, `docs/SPEC.md`, integration tests.
- **Approach:** Atomically add `graph/for-each-unit@1` and `agent/repository-skill@1` to the generated supervisor/sandbox runtime descriptor and install their parent-stage dispatch seams. Expose an authenticated, bounded supervisor capability response carrying the active runtime release, capability digest, and capability IDs. Replace RU2's unconditional structured-shipping failure with a pre-mutation check that requires the exact structured capability from the configured supervisor; unreachable, missing, stale, or mismatched evidence fails before Linear access and never falls back to `simple`. Construct the child effect processor in the composition root and drain its worktree, repository-skill materialization, loop, command, candidate, integration, stop, and cleanup intents. Bind every result to the composite parent run. Count only accepted integrated units as successful aggregate inputs. Route exited or incomplete graphs to their typed non-success outcome. Update the supervisor HTTP and activation contract in `docs/SPEC.md` in the same PR.
- **Test Scenarios:** A two-unit in-process integration test advances through the real operations ports. Restart at each intent boundary neither duplicates nor skips work. The CLI makes no Linear request when capability discovery is unreachable, unauthenticated, missing, stale, or mismatched, and permits structured mutation only for matching active evidence. One exited unit cannot yield success. Aggregate and parent stage result settle exactly once.
- **Verification:** Run composition/architecture, HTTP capability, CLI ship, operations, coordinator, persistence, runtime event, and Daytona adapter tests; supervisor and CLI typecheck/build; `git diff --check docs/SPEC.md`.
...
- **Approach:** Add a host-side walking-skeleton harness that composes built supervisor child coordinator/effect modules with a test-only provider-neutral runtime adapter whose methods invoke the built container's sealed worktree, loop, command, result-collection, integration, and cleanup executors. Keep all reduction and gate decisions in production modules. Run two ordered stub units through implement, simplify, commands, executor candidate derivation, lead acceptance of that exact candidate, integration, full commands, one fresh final review, and one aggregate parent result. Assert unit 2 starts at unit 1's exact integrated head. Use no Linear, Daytona, Fly, GitHub, or model credentials. Do not add rollout help or new CI wiring in this slice unless an existing assertion must be corrected for the local smoke to run.
...
- **Goal:** Make Linear and GitHub a restart-safe projection of durable child progress, then converge the remaining documentation and rerun the complete local proof.
...
| CLI | RU1-RU3, RU9, RU11 | `npm run typecheck --prefix cli && npm run build --prefix cli && npm test --prefix cli` |
...
| Normative contract | RU4, RU9, and any contract-changing unit | `docs/SPEC.md` changes in the owning PR and matches its regression tests; `git diff --check docs/SPEC.md` passes. |
...
- Before RU9 capability evidence is live, explicit structured shipping fails before Linear mutation and never falls back to `simple`.
