...
>
> - **The default implement pipeline is `core/implement@4`, and its true shape is 11 stages, not the 9 the prior draft named.** `supervisor/pipelines/catalog.yaml` aliases `implement → core/implement@4`; `supervisor/pipelines/core-implement-v4.yaml` runs, in order: **implementation → repair_implementation → repair_semantic_review → semantic_review → simplification → post_simplify_review → test → lint → build → publish → provider**. The two `repair_*` stages are **conditional**: they are entered only when `implementation` returns `semantic_repair_required`/`failure`, and `repair_semantic_review` **rejoins the forward path at `test`** (skipping simplification, which the initial forward pass already ran). `post_simplify_review` is **conditional** too: `simplification` success routes to it, but a `no_change` simplification skips straight to `test`. Repair is bounded by a **global round-based budget** `max_repair_rounds: 5` (tracked as `instance.reentry_count`), **scoped per-transition** `max_reentries` (tracked as `targetState.reentry_count`), and a **whole-run net attempt cap** `max_attempts: 200` (tracked as `instance.attempt_count`). Every "current simple manifest = `core/implement@1/2`", "8-stage flow", or "whole-plan `ce-work mode:return-to-caller`" baseline reference has been retargeted to this shape.
...
1. `simple` passes the complete approved plan through the existing staged CE flow (`core/implement@4`: implementation → repair_implementation → repair_semantic_review → semantic_review → simplification → post_simplify_review → test → lint → build → publish → provider, where the two `repair_*` stages and `post_simplify_review` are conditional and repair is bounded by the round-based `max_repair_rounds` budget) in one continuing agent context. Its compiled manifest must be **behaviorally equivalent to `core/implement@4`**, re-pinned to a new digest.
...
- `supervisor/pipelines/catalog.yaml` and `supervisor/pipelines/core-implement-v4.yaml` — the current default (`implement → core/implement@4`), whose true shape is 11 stages (implementation → repair_implementation → repair_semantic_review → semantic_review → simplification → post_simplify_review → test → lint → build → publish → provider) with conditional `repair_*` stages (entered on `semantic_repair_required`/`failure`, rejoining at `test`), a conditional `post_simplify_review`, and the round-based `max_repair_rounds: 5` / per-transition `max_reentries` / `max_attempts: 200` budgets the `simple` graph must reproduce **behaviorally** (re-pinned to a new digest, not byte-for-byte).
...
- `docs/SPEC.md` §persistence — the reserved `pipeline_work_bindings` junction Stage C first writes, and the migration model Stage C extends as additive v16 (latest merged is v15).
...
4. Make the built-in simple graph compile to current implement behavior, including its existing context and repair rules, preserving the conditional `repair_*`/`post_simplify_review` stages and their rejoin-at-`test` transitions. **Emulate `core/implement@4`'s global round-based repair budget (`max_repair_rounds`) via a scoped repair stage bounded by reentry count** — today's coordinator has no first-class "round" primitive (a "round" is itself counted as `instance.reentry_count` against `max_repair_rounds`), so the compiler expresses the round budget through bounded `max_reentries` on the repair transitions rather than a new primitive. Do not silently reorder simple review/simplification in this unit.
...
**Requirements:** R12–R15, R33–R34; F3; AE3–AE4; KTD4 (V1 half — no in-place shape change), KTD7, KTD11, KTD13.

**Dependencies:** U3.

**Files:**

- Modify `supervisor/src/persistence/migrations/definitions.ts`, `supervisor/src/persistence/schema.ts`, and `supervisor/src/persistence/migrations/runner.test.ts` — add child graph/unit/session/context/gate/effect records as **additive migration v16** (v15 `orchestration-journal` is the latest merged; all new tables are checksum-pinned and never rewrite history). Reserve the net-new `exited` terminal level + alarm bit column on `execution_units` (R39, realized in U4c). *(The R40 reserve budget cursor on `execution_work_attempts` is deferred with the autonomy layer.)* Stage C becomes the first writer of the SPEC-reserved `pipeline_work_bindings` junction, binding each unit delivery to its fenced pipeline attempt/unit.
