...
name: review-fix
description: >
  Applies requested PR review changes: reads the feedback, fixes each
  item, runs test/lint/format, pushes to the existing branch (no new PR),
  and posts a Linear update. Use when a PR has "request changes" review
  feedback to address.
---
...
5. Push to `${BRANCH_NAME}`. **Do not create a new PR** — this updates
   the existing one.
6. Post a short `action` activity to the Linear session
   (`LINEAR_SESSION_ID`) summarizing what was fixed and inviting a
   re-review reply — e.g., "Addressed the review — pushed 3 commits.
