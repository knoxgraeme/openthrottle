...
2. Post exactly one concise `gh pr comment` containing the blocking findings,
   ...
   Tag any finding whose resolution needs a human decision — one that is
   critical, foundational, or risky, or has more than one defensible fix — as
   `decision-required` so the fix run escalates it instead of guessing.
