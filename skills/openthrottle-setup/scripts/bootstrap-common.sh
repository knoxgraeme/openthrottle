...
# ---------------------------------------------------------------------------
# Managed autostart — all service entries in one file, sourced by .bashrc
# ---------------------------------------------------------------------------
AUTOSTART_FILE="${SPRITE_HOME}/start-services.sh"

init_autostart() {
  # Write a fresh autostart file (idempotent — always overwrites)
  cat > "$AUTOSTART_FILE" << 'INITEOF'
#!/usr/bin/env bash
# Auto-generated by bootstrap — start background services.
SPRITE_HOME="${SPRITE_HOME:-/home/sprite}"
INITEOF
  chmod +x "$AUTOSTART_FILE"

  # Source it from .bashrc (once, idempotent)
  local BASHRC="${HOME}/.bashrc"
  if ! grep -q "start-services.sh" "$BASHRC" 2>/dev/null; then
    echo "source ${AUTOSTART_FILE}" >> "$BASHRC"
  fi
  log "Autostart file initialized at ${AUTOSTART_FILE}"
}
...
add_autostart() {
  ...
  # Guard against duplicate entries if called without init_autostart
  if grep -q "$script_name" "$AUTOSTART_FILE" 2>/dev/null; then
    log "Auto-start already configured for ${script_name}"
    return 0
  fi

  cat >> "$AUTOSTART_FILE" << ENTRY
