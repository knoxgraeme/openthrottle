...
8. Resolve the PR URL and base with
   ...
   before continuing.
9. If CE returned needs-human residuals that a specific answer would unblock,
...
adapter's job to chase: it arrives later as a `resume` message in this same
session, carrying the feedback to triage. When that happens: action clear
fixes and push, answer non-actionable threads on their own thread with your
reasoning, and batch any decision-required items into one further
elicitation. Never leave an item unaddressed across a resume.
